...
def show_total(img, ObjectID):
    """Draw ObjectID number at top-left corner of the image."""
    font = cv2.FONT_HERSHEY_PLAIN
    line = cv2.LINE_AA
    total = 'total ID {}'.format(ObjectID)
    cv2.putText(img, total, (111,20), font, 1.0, (32, 32, 32), 4, line)
    cv2.putText(img, total, (110,20), font, 1.0, (240, 240, 240), 1, line)
    return img

def show_model_ID(img, model_ID):
    """Draw ObjectID number at top-left corner of the image."""
    font = cv2.FONT_HERSHEY_PLAIN
    line = cv2.LINE_AA
    model = 'model name: {}'.format(model_ID)
    cv2.putText(img, model, (11, 40), font, 1.0, (32, 32, 32), 4, line)
    cv2.putText(img, model, (10, 40), font, 1.0, (240, 240, 240), 1, line)
    return img
...
class ScreenToggler():
    ...
    def toggle(self):
        ...
        set_display( self.WINDOW_NAME, self.full_scrn)
