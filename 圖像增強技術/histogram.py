...
# img = cv.imread('Jetplane.bmp', cv.IMREAD_GRAYSCALE)
# img = cv.imread("Peppers.bmp", cv.IMREAD_GRAYSCALE)
img = cv.imread('test/371297900142265427.jpg', cv.IMREAD_GRAYSCALE)
...
for i in range(height):
    for j in range(width):
        h_eq[i][j] = (
            (cdf[img[i][j]] - np.min(cdf))
            / ((height * width) - np.min(cdf))
            * (256 - 1)
        )
